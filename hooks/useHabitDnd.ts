...
export function useHabitDnd({ habits, onHabitMoved, onCompleteHabit, onUnmarkConfirmation }: UseHabitDndProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (Object.values(HabitBoxType).includes(overId as HabitBoxType)) {
      ...
      if (overHabit) {
        if (overHabit.habit_state === HabitState.TODAY) targetBox = HabitBoxType.TODAY;
        else if (overHabit.habit_state === HabitState.YESTERDAY) targetBox = HabitBoxType.YESTERDAY;
        else targetBox = HabitBoxType.PILE;
      }
    }
    ...
    if (habit.habit_state === HabitState.TODAY) sourceBox = HabitBoxType.TODAY;
    else if (habit.habit_state === HabitState.YESTERDAY) sourceBox = HabitBoxType.YESTERDAY;
    ...
    if (targetBox === HabitBoxType.TODAY) newHabitState = HabitState.TODAY;
    else if (targetBox === HabitBoxType.YESTERDAY) newHabitState = HabitState.YESTERDAY;
    else newHabitState = HabitState.LIVELY; // Default for Pile
    ...
    try {
      if (sourceBox === HabitBoxType.TODAY && (targetBox === HabitBoxType.YESTERDAY || targetBox === HabitBoxType.PILE)) {
        const performUnmark = async () => {
             await unmarkHabit(habitId, newHabitState);
             onHabitMoved?.();
        };
