...
interface HabitInfoModalProps {
    ...
    isOpen: boolean;
    onClose: () => void;
    onHabitUpdated?: (habitId: string, name: string, isPublic: boolean, goalValue?: number | null, goalUnit?: string | null, targetTime?: string | null, // Add targetTime
...
const HabitInfoModal: React.FC<HabitInfoModalProps> = ({
    ...
    const handleSave = (habitId: string, name: string, isPublic: boolean, goalValue?: number | null, goalUnit?: string | null, targetTime?: string | null, // Add targetTime
                        description?: string | null // Add description
    ) => {
    ...
    return (<BaseModal
            ...
            footerContent={isPrivateHabit && (<div className="flex flex-col w-full gap-3 pt-2">
                        ...
                        <div className="flex flex-wrap gap-3 justify-center pb-4 border-b border-border/40 mb-2">
                            {habit.habit_state !== HabitState.TODAY ? (
                                <Button variant="outline" size="sm" onClick={() => handleMove(HabitBoxType.TODAY)}
                                    ...
                                    {onRedeemComplete && (<Button variant="secondary" size="sm" onClick={() => {
                                            onClose();
                                            onRedeemComplete();
                                        }} className="w-full sm:w-auto">
        ...
        >
            <div className="flex flex-col gap-6 pb-2 px-1">
                 ...
                 <span className="capitalize flex items-center gap-1">
                    ...
                    {habit.linked_identities && habit.linked_identities.length > 0 && (
                        <div className="flex flex-wrap justify-center gap-2 mt-2 pt-1">
                            {habit.linked_identities.map(identity => (<IdentityChip
                                    key={identity.id}
                                    id={identity.id}
                ...
                <div
                    className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent rounded-2xl p-6 flex flex-col items-center justify-center border border-primary/10 shadow-sm relative overflow-hidden shrink-0">
                ...
                {/* Description */}
                {habit.descriptions && (<div className="bg-muted/40 p-4 rounded-xl border border-border/50 shrink-0">
                        <span
                            className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 block">Why this matters</span>
                ...
                <div className="grid grid-cols-2 gap-3 shrink-0">
                    ...
                    <div className="bg-card border p-3 rounded-xl flex flex-col gap-1 shadow-sm">
                     ...
                     {habit.created_at ? new Date(habit.created_at).toLocaleDateString(undefined, {
                         month: 'short',
                         day: 'numeric',
                         year: '2-digit'
                     }) : 'N/A'}
                    ...
                    {habit.target_time && (<div className="bg-card border p-3 rounded-xl flex flex-col gap-1 shadow-sm">
                            <span className="text-[10px] text-muted-foreground uppercase font-bold">Target Time</span>
                            <span className="text-lg font-semibold">
                        {new Date(`1970-01-01T${habit.target_time}`).toLocaleTimeString([], {
                            hour: 'numeric',
                            minute: '2-digit'
                        })}
                     </span>
                    ...
                    {(habit.goal_value && habit.goal_unit) && (
                        <div className="bg-card border p-3 rounded-xl flex flex-col gap-1 shadow-sm">
                            ...
                            <span className="text-lg font-semibold">
                        {habit.goal_value} <span
                                className="text-sm font-normal text-muted-foreground">{habit.goal_unit}</span>
                     </span>
                        </div>)}
                </div>
            </div>

            {isPrivateHabit && (<EditHabitModal
