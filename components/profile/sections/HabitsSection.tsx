...
export const HabitsSection: React.FC<HabitsSectionProps> = ({
    ...
    const baseHabits =
        propHabits ?? (isOwner ? mockHabitsData : mockPublicHabitsData);

    const [activeHabitForCompletion, setActiveHabitForCompletion] =
    ...
    const handleUnmarkConfirmation = (
        habit: Habit,
        onConfirm: () => Promise<void>,
        onCancel: () => void
    ) => {
    ...
    const yesterdayHabits = habits.filter(
        (h) => h.habit_state === HabitState.YESTERDAY
    );
    const pileHabits = habits.filter(
        (h) =>
            h.habit_state === HabitState.LIVELY || h.habit_state === HabitState.JUNKED
    );

    const [showAllPileHabits, setShowAllPileHabits] = useState(false);
    const initialVisibleHabits = 5;
    const habitsToDisplay = showAllPileHabits
        ? pileHabits
        : pileHabits.slice(0, initialVisibleHabits);
    ...
    const renderHabitChip = (h: Habit) => (
        <HabitChip
            ...
            box={
                h.habit_state === HabitState.TODAY
                    ? HabitBoxType.TODAY
                    : h.habit_state === HabitState.YESTERDAY
                        ? HabitBoxType.YESTERDAY
                        : HabitBoxType.PILE
            }
        />
    );
